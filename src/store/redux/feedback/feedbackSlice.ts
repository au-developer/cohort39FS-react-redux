...
//ШАГ-3 значение по умолчанию для состояния Компонента
...
// ШАГ-2 создание Слайса через вызов импортированой функции с обязательными ключами name, initialState
// ШАГ-6 експорт слайса - ШАГ-7,8 в store.ts сделать импорт и передать в аргкмент функции combineSlices
export const feedbackSlice = createAppSlice({
  name: "FEEDBACK", // используем в тулбаре браузера для отслеживания Слайса, и имя для Екшена
  //ШАГ-4 передача изначального значения для инициализации, значнее по умолчанию для этого slice, initialState
  initialState: feedbackInitialState,
  // ШАГ-5 Добавления функции редьюсера - callBack с аргументом create, который и создает reducers-->>функции, 
  //изменяющие состояние counterSlice
  reducers: create => {   // create --> инструмент из библиотеки
    return {
      like: create.reducer(
        //ШАГ-11 типизация СТейта
